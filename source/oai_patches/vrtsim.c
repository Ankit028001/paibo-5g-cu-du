...
typedef struct {
  ...
  // Set true (server side only) once the IPC handshake for that ue_id has completed; used to gate
  // the per-UE TX/RX work in vrtsim_write()/vrtsim_read() on actual connection state instead of the
  // configured num_ues. Atomic: written by vrtsim_ipc_thread, read by the TX/RX callback threads.
  // Zero-initialized (all false) via the existing calloc_or_fail() allocation in device_init().
  _Atomic bool ue_connected[MAX_NUM_UES];
} vrtsim_state_t;
...
static void *vrtsim_ipc_thread(void *arg)
{
  ...
  while (vrtsim_state->run_ipc_thread) {
    ...
    // Read the connecting client's ue_id before publishing client_info, so we can mark that
    // slot as actually connected (used to gate per-UE TX/RX work; see ue_connected).
    int peer_ue_id = -1;
    size_t id_read = 0;
    bool id_read_ok = true;
    char *id_ptr = (char *)&peer_ue_id;
    while (id_read < sizeof(peer_ue_id)) {
      ssize_t r = read(client_fd, id_ptr + id_read, sizeof(peer_ue_id) - id_read);
      if (r < 0) {
        if (errno == EINTR) {
          continue;
        }
      ...
      if (r == 0) {
        // Peer closed before sending its ue_id: incomplete handshake, not a valid UE ID.
        id_read_ok = false;
        break;
      }
      id_read += r;
    ...
    if (id_read_ok && id_read == sizeof(peer_ue_id) && peer_ue_id >= 0 && peer_ue_id < vrtsim_state->num_ues) {
      atomic_store(&vrtsim_state->ue_connected[peer_ue_id], true);
    } else {
      LOG_W(HW, "VRTSIM: IPC handshake did not yield a valid ue_id (got %d); not marking any UE connected\n", peer_ue_id);
    }
...
static size_t try_write_ue_id(int fd, int ue_id)
{
  size_t total_sent = 0;
  const char *ptr = (const char *)&ue_id;
  while (total_sent < sizeof(ue_id)) {
    ssize_t sent = write(fd, ptr + total_sent, sizeof(ue_id) - total_sent);
    if (sent < 0) {
      if (errno == EINTR) {
        continue;
      }
...
static client_info_t client_read_info(char *socket_path, uint32_t connection_timeout, int ue_id)
{
  client_info_t client_info;
  uint32_t tries = 0;
  struct sockaddr_un addr;
  ...
  while (connection_timeout == 0 || tries < connection_timeout) {
    ...
    if (sock_fd >= 0) {
      if (connect(sock_fd, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
        // Tell the server which ue_id this client is claiming, before it publishes client_info,
        // so the server can gate per-UE TX/RX work on actual connection state (see ue_connected).
        size_t id_sent = try_write_ue_id(sock_fd, ue_id);
        if (id_sent != sizeof(ue_id)) {
          close(sock_fd);
          sleep(1);
          tries++;
          continue;
...
static int vrtsim_connect(openair0_device_t *device)
{
  ...
  if (vrtsim_state->role == ROLE_SERVER) {
    ...
    client_info_t client_info =
        client_read_info(vrtsim_state->connection_descriptor, vrtsim_state->connection_timeout, vrtsim_state->ue_id);
                        ...
                        int flags)
{
  ...
  if (vrtsim_state->role == ROLE_CLIENT) {
    ...
    for (int u = 0; u < vrtsim_state->num_ues; u++) {
      if (!atomic_load(&vrtsim_state->ue_connected[u]))
        continue;
