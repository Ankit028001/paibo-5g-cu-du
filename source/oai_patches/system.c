...
void threadCreate(pthread_t* t, void * (*func)(void*), void * param, char* name, int affinity, int priority)
{
  ...
  if (ret == EPERM && set_prio) {
    /* PATCH#2 WSL2/restricted kernels forbid SCHED_RR/FIFO even with CAP_SYS_NICE.
       Retry once with default (SCHED_OTHER) scheduling. Self-paced rfsim runs are
       unaffected; hard real-time guarantees for real radios are lost. */
    LOG_W(UTIL, "%s: RT sched denied (EPERM) for '%s'; using default scheduling\n", __func__, name);
    pthread_attr_t a2;
    pthread_attr_init(&a2);
    if (affinity != -1) {
      cpu_set_t cs2; CPU_ZERO(&cs2); CPU_SET(affinity, &cs2);
      pthread_attr_setaffinity_np(&a2, sizeof(cpu_set_t), &cs2);
    }
    ret = pthread_create(t, &a2, func, param);
